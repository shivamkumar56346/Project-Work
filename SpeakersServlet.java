...
public class SpeakersServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet SpeakersServlet</title>");            
            out.println("</head>");
            out.println("<body bgcolor='orange'>");
            out.println("<center>");
            out.println("<h1><u>"+greet+"</h1>");
            out.println("<span style='color:blueviolet'><b>Admin Speakers Gallery!!</span>");
            out.println("<div style='background-color:yellow'><hr>");
            
            int catId=Integer.parseInt(request.getParameter("catId"));
            try{
            Connection conn=DatabaseConnect.getMyConnection();
            PreparedStatement pre=conn.prepareStatement("select * from productsinfo where Product_Cat_Id=?");
            pre.setInt(1, catId);
            ...
            out.println("<td><img src='"+rs.getString(5)+"' width='120' height='80'/></td>");
